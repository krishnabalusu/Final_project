...
def main():
    ...
    st.write('Purpose: The country dropdown menu allows users to select a specific country from the dataset.
Functionality: By selecting a country, users can focus the analysis on sales data from that particular country. This enables them to understand sales performance, product distribution, and trends within a specific market.
Use Case: Users may want to analyze sales data for different countries to identify regions with the highest sales volumes, explore market preferences for certain products, or tailor marketing strategies to specific countries.')
